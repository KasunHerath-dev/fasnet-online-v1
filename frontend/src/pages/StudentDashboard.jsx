...
const StatCard = ({ icon: Icon, value, label, color = '#f3184c' }) => (
    <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl p-4 sm:p-6 border border-gray-100 dark:border-[#303030] hover:shadow-lg transition-all">
...
const EventCard = ({ title, date, time, type }) => (
    <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-gray-50 dark:bg-[#303030] rounded-xl hover:bg-gray-100 dark:hover:bg-[#3a3a3a] transition-colors group cursor-pointer">
        ...
        <div className="flex-1 min-w-0">
            <h4 className="font-bold text-sm sm:text-base text-gray-900 dark:text-white truncate group-hover:text-[#f3184c] transition-colors">{title}</h4>
...
const SubjectCard = ({ title, code, credits, onClick }) => (
    <div
        onClick={onClick}
        className="bg-white dark:bg-[#1e1e1e] rounded-xl p-4 border border-gray-100 dark:border-[#303030] hover:border-[#f3184c] hover:shadow-md transition-all cursor-pointer group"
    >
        <div className="flex items-start justify-between mb-3">
            <div className="p-2 bg-gray-100 dark:bg-[#303030] rounded-lg group-hover:bg-[#f3184c]/10 transition-colors">
                <BookOpen className="w-5 h-5 text-[#f3184c]" />
            </div>
            <span className="text-xs font-bold text-gray-400">{credits} Credits</span>
        </div>
        <h4 className="font-bold text-gray-900 dark:text-white text-sm mb-1 truncate">{title}</h4>
...
export default function StudentDashboard() {
    ...
    const fetchData = async (studentRef) => {
        try {
            ...
            // Get modules for student's level
            ...
            // Fetch academic records for charts
            try {
                const records = await academicService.getAcademicRecords()
                if (records && records.length > 0) {
                    // Process records for chart
                    const chartData = records.slice(-6).map(record => ({
                        semester: record.semester || 'S' + record.level,
    ...
    // Grade distribution from academic data
                    ...
                    {activeTab === 'overview' && (
                        <div className="space-y-4 sm:space-y-6">
                            ...
                            <div className="grid lg:grid-cols-3 gap-4 sm:gap-6">
                                ...
                                {/* Upcoming Events */}
                                <div className="lg:col-span-2 bg-white dark:bg-[#1e1e1e] rounded-2xl p-4 sm:p-6 border border-gray-100 dark:border-[#303030]">
                                    <div className="flex items-center justify-between mb-4 sm:mb-6">
                                        <h2 className="text-lg sm:text-xl font-black text-gray-900 dark:text-white">Upcoming Events</h2>
                                        <button className="text-xs font-bold text-[#f3184c] hover:text-[#d01440] transition-colors">View All</button>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    {/* Degree Progress */}
                                    <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl p-4 sm:p-6 border border-gray-100 dark:border-[#303030]">
                                        ...
                                        <div className="w-full h-3 bg-gray-200 dark:bg-[#303030] rounded-full overflow-hidden">
                                            <div
