...
export default function TopNav({ user, onLogout, onToggleSidebar }) {
  ...
  return (
    <header className="sticky top-4 z-40 px-4 md:px-8">
      <div className="bg-white/80 dark:bg-[#121212]/80 backdrop-blur-xl rounded-[2rem] shadow-lg border border-white/20 dark:border-white/10 px-6 py-3 transition-all duration-300">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="md:hidden p-2 hover:bg-white/10 rounded-xl transition-colors text-slate-800 dark:text-white"
            >
          ...
          <div className="flex-1 max-w-2xl mx-auto hidden md:block">
            <div className="relative group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-[#f3184c] transition-colors" />
              <input
                type="text"
                placeholder="What assignment are you looking for?"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full bg-slate-100/50 dark:bg-black/40 hover:bg-slate-100 dark:hover:bg-black/60 focus:bg-white dark:focus:bg-black border border-transparent focus:border-[#f3184c]/30 rounded-2xl py-2.5 pl-11 pr-4 text-sm font-bold text-slate-700 dark:text-slate-200 outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 ml-auto">
            ...
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="w-10 h-10 bg-slate-100/50 dark:bg-white/5 hover:bg-white dark:hover:bg-white/10 rounded-xl flex items-center justify-center transition-all border border-transparent hover:border-slate-200 dark:hover:border-white/10"
              >
                <Bell className="w-5 h-5 text-slate-600 dark:text-slate-300" />
                {unreadCount > 0 && (
                  <span className="absolute top-2 right-2.5 w-2 h-2 bg-[#f3184c] rounded-full ring-2 ring-white dark:ring-[#121212]"></span>
                )}
              </button>

              {/* Notification Dropdown (Glass) */}
              {showNotifications && (
                <div className="absolute right-0 mt-4 w-80 bg-white/90 dark:bg-[#121212]/90 backdrop-blur-2xl border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-4 duration-200">
            ...
            <div className="relative" ref={userMenuRef}>
              <button
                ...
                className="flex items-center gap-3 bg-slate-100/50 dark:bg-white/5 hover:bg-slate-200/50 dark:hover:bg-white/10 rounded-xl pl-1 pr-3 py-1 transition-all border border-transparent hover:border-slate-200 dark:hover:border-white/10"
              >
                <div className="w-8 h-8 bg-gradient-to-br from-[#f3184c] to-[#ff4d73] rounded-lg flex items-center justify-center shadow-lg shadow-pink-500/20">
                  <span className="text-white font-black text-xs">{initials}</span>
                </div>
                <ChevronDown className={`w-3 h-3 text-slate-500 dark:text-slate-400 transition-transform ${showUserMenu ? 'rotate-180' : ''}`} />
              </button>
